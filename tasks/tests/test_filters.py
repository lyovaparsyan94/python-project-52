...
class TaskFilterViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.status_new = Status.objects.create(
            name='Новая'
        )
        cls.status_done = Status.objects.create(
            name='Выполнена'
        )

        cls.label_frontend = Label.objects.create(
            name='Frontend'
        )
        cls.label_backend = Label.objects.create(
            name='Backend'
        )
    ...
    def test_filter_is_owner(self):
        ...
        response = self.client.get(
            reverse('tasks:tasks'), {'is_owner': 'on'}
        )
